...
/**
  * @brief  Returns the quantization scheme used to quantize the neural netwrok
  * @param  None
  * @retval ai_size   Quantization scheme: AI_FXP_Q, AI_UINT_Q, AI_SINT_Q
  */
uint32_t ai_get_quantization_scheme(void)
{
  ai_float scale=ai_get_input_scale_offset();
  
  /*temp*/
 /* ai_network_report report;
  ai_network_get_info(network_handle, &report);
  
  ai_buffer *input = &report.inputs[0];*/
  /**/
  
  ai_buffer_format fmt=AI_BUFFER_FORMAT(/*input*/&ai_input[0]);
...
/**
  * @brief  Returns value of the scale offset for the input quantized format
  * @param  None
  * @retval ai_size   Scale offset for input quantized format
  */
ai_float ai_get_input_scale_offset(void)
{
  /*ai_network_report report;
  ai_network_get_info(network_handle, &report);
  
  ai_buffer *input = &report.inputs[0];*/
  return AI_BUFFER_META_INFO_INTQ_GET_SCALE(/*input->meta_info*/ai_input[0].meta_info, 0);
}
