...
void OnBoardValid(void)
{
  ...
  while (STM32Fs_GetNextDir(&dataset_dir, &fno) == STM32FS_ERROR_NONE)
  {
    ...
    while (STM32Fs_GetNextFile(&class_dir, &img_fno) == STM32FS_ERROR_NONE)
    {
      ...
      /*Pixel value convertion and normalisation W/out RB swap*/
      ai_pixel_conversion((void*)(rgb24_scaled_image_buff), (void*)(nn_input_buff), ai_get_input_width() * ai_get_input_height(), 0, pixel_conv_lut);
